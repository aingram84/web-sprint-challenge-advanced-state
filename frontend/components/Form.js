...
const formSchema = object().shape({
  newQuestion: string().min(2).trim().required("Must include question!"),
  newTrueAnswer: string().min(2).trim().required("Must include True answer!"),
  newFalseAnswer: string().min(2).trim().required("Must include False answer!")
})
...
export function Form(props) {
  ...
  const onChange = evt => props.inputChange(evt);
